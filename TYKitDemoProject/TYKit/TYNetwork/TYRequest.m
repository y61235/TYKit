...
- (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        NSTYLog(@"create cache directory failed, error = %@", error);
    } else {
//        [TYNetworkPrivate addDoNotBackupAttribute:path];
    }
}
...
- (NSString *)cacheBasePath {
    ...
    NSArray *filters = [[TYNetworkConfig sharedInstance] cacheDirPathFilters];
    if (filters.count > 0) {
        for (id<TYCacheDirPathFilterProtocol> f in filters) {
...
- (NSString *)cacheFileName {
    ...
    NSString *requestInfo = [NSString stringWithFormat:@"Method:%ld Host:%@ Url:%@ Argument:%@ AppVersion:%@ Sensitive:%@",
                             ...
                             argument, [TYNetworkUtil appVersionString], [self cacheSensitiveData]];
    NSString *cacheFileName = [TYNetworkUtil md5StringFromString:requestInfo];
...
- (int)cacheFileDuration:(NSString *)path {
    ...
    if (!attributes) {
        NSTYLog(@"Error get attributes for file at %@: %@", path, attributesRetrievalError);
        return -1;
    }
